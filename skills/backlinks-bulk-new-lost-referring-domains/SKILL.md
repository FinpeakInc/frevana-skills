---
name: backlinks-bulk-new-lost-referring-domains
description: Use when the user wants new and lost external referring-domain count totals for many domains or URLs in one batch request.
---

# Backlinks Bulk New Lost Referring Domains

Call Frevana's `/service/backlinks/bulk-new-lost-referring-domains` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **new and lost referring-domain count totals for many domains or URLs in one batch request**.
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_new_lost_referring_domains.sh --targets frevana.com,example.com --date-from 2026-06-01
```
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_new_lost_referring_domains.sh --targets frevana.com,example.com --date-from 2026-06-01 \
  --output ./out/backlinks-bulk-new-lost-referring-domains-result.json
```
...
```text
POST /service/backlinks/bulk-new-lost-referring-domains
```

Supported request fields for this API:

`targets`, `date_from`, `tag`
...
- Default file path: `./out/backlinks-bulk-new-lost-referring-domains-<UTC timestamp>-<pid>.json`
