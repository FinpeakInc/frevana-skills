---
name: deepseek-ask
description: Use when the user wants to ask DeepSeek a question through Frevana using the user's logged-in Chrome Extension session.
---

# DeepSeek Ask

Ask DeepSeek through the local Frevana daemon and Chrome Extension session.

## Purpose

This skill is for **asking DeepSeek a prompt** using the Chrome Extension-backed Frevana MCP tool `frevana_ask` with fixed provider `deepseek`.

Inputs:

- `prompt` (required) - one or more questions or instructions to send to DeepSeek
...
- DeepSeek's answer returned by Frevana
...
- the user logged in to DeepSeek in Chrome
- `curl`
- `bash`
- `python3`
...
6. Run the DeepSeek ask only after setup succeeds.
7. If the call returns login/auth content or fails because DeepSeek is unavailable, tell the user they must be logged in to DeepSeek in Chrome.
...
```bash
bash <skill-path>/scripts/ask_deepseek.sh \
...
bash <skill-path>/scripts/ask_deepseek.sh \
  ...
  --timeout 120000 \
  --output ./out/deepseek-answer.txt
```
...
bash <skill-path>/scripts/ask_deepseek.sh \
  ...
  --format json \
  --output ./out/deepseek-batch-answers.txt
```
...
Always send `provider: "deepseek"`. Do not pass unsupported fields.
...
- "Ask DeepSeek to explain quantum computing"
- "用 DeepSeek 回答这个问题：..."
