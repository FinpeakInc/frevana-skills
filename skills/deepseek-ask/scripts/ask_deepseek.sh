...
usage() {
  cat <<'EOF'
Usage:
  ask_deepseek.sh --prompt "question 1" [--prompt "question 2"] [--prompt-file /path/to/prompts.txt] [--format text|json] [--timeout MS] [--output /path/to/result.txt]
...
if [[ -n "$OUTPUT_PATH" ]]; then
  ...
  echo "Saved DeepSeek answer to $OUTPUT_PATH" >&2
fi
