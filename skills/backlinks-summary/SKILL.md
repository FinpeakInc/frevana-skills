...
description: Use when the user needs high-level external backlink/inbound-link totals and authority metrics for one domain, subdomain, or URL without listing individual links.
---

# Backlinks Summary

Call Frevana's `/service/backlinks/summary` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **high-level backlink totals and authority metrics for one domain, subdomain, or URL without listing individual backlinks**.
...
```bash
bash <skill-path>/scripts/get_backlinks_summary.sh --target frevana.com
```
...
```bash
bash <skill-path>/scripts/get_backlinks_summary.sh --target frevana.com \
  --output ./out/backlinks-summary-result.json
```
...
```text
POST /service/backlinks/summary
```

Supported request fields for this API:

`target`, `include_subdomains`, `include_indirect_links`, `exclude_internal_backlinks`, `internal_list_limit`, `backlinks_status_type`, `backlinks_filters`, `rank_scale`, `tag`
...
- Default file path: `./out/backlinks-summary-<UTC timestamp>-<pid>.json`
