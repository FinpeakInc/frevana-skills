...
usage() {
  cat <<'EOF'
Usage:
  get_amazon_rufus_qa.sh --url "https://www.amazon.com/dp/B0XXXXXXXX" [--format text|json] [--timeout MS] [--output /path/to/result.txt]
...
if [[ -n "$OUTPUT_PATH" ]]; then
  ...
  echo "Saved ${LABEL} result to $OUTPUT_PATH" >&2
fi
