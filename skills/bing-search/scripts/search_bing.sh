...
usage() {
  cat <<'EOF'
Usage:
  search_bing.sh --q VALUE [options] [--output /path/to/result.json] [--token "bearer token"]
...
Request options:
  ...
  --lat                        float parameter
  --lon                        float parameter
  --mkt                        string parameter
  --cc                         string parameter
  --setLang                    string parameter
  --first                      int parameter
  --count                      int parameter
  --safeSearch                 string parameter
  --filters                    string parameter
...
python3 - "$PAYLOAD_FILE" '{"q":"q","location":"location","lat":"lat","lon":"lon","mkt":"mkt","cc":"cc","setLang":"setLang","first":"first","count":"count","safeSearch":"safeSearch","filters":"filters","device":"device","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"q":"string","location":"string","lat":"float","lon":"float","mkt":"string","cc":"string","setLang":"string","first":"int","count":"int","safeSearch":"string","filters":"string","device":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["q"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Bing Search API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Bing Search API returned an empty response body."
