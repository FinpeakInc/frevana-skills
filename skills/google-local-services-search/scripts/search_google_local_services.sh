...
API_PATH="/service/google-local-services"
CONNECT_TIMEOUT="10"
MAX_TIME="600"
...
usage() {
  cat <<'EOF'
Usage:
  search_google_local_services.sh --q VALUE --data-cid VALUE [options] [--output /path/to/result.json] [--token "bearer token"]

Request options:
  --q                          string parameter
  --data-cid                   string parameter
  --hl                         string parameter
  --job-type                   string parameter
  --cid                        string parameter
  --bid                        string parameter
  --pid                        string parameter
...
python3 - "$PAYLOAD_FILE" '{"q":"q","data_cid":"data_cid","data-cid":"data_cid","hl":"hl","job_type":"job_type","job-type":"job_type","cid":"cid","bid":"bid","pid":"pid","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"q":"string","data_cid":"string","hl":"string","job_type":"string","cid":"string","bid":"string","pid":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["q","data_cid"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google Local Services Search API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Google Local Services Search API returned an empty response body."
