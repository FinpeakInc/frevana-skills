...
API_PATH="/service/google-ai-mode"
CONNECT_TIMEOUT="10"
MAX_TIME="600"
...
usage() {
  cat <<'EOF'
Usage:
  search_google_ai_mode.sh --q VALUE [options] [--output /path/to/result.json] [--token "bearer token"]
...
Request options:
  ...
  --gl                         string parameter
  --hl                         string parameter
  --next-page-token            string parameter
  --continuable                bool parameter
  --subsequent-request-token   string parameter
  --image-url                  string parameter
...
python3 - "$PAYLOAD_FILE" '{"q":"q","location":"location","uule":"uule","gl":"gl","hl":"hl","next_page_token":"next_page_token","next-page-token":"next_page_token","continuable":"continuable","subsequent_request_token":"subsequent_request_token","subsequent-request-token":"subsequent_request_token","image_url":"image_url","image-url":"image_url","device":"device","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"q":"string","location":"string","uule":"string","gl":"string","hl":"string","next_page_token":"string","continuable":"bool","subsequent_request_token":"string","image_url":"string","device":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["q"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google AI Mode API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Google AI Mode API returned an empty response body."
