# TikTok Posts Discover by Profile URL API contract

This reference mirrors the current `frevana-server` controller and request DTO.

## Endpoints

- Create: `POST /service/tiktok/posts/discover-by-profile-url`
...
```json
{
  ...
  "limit_per_input": 100,
  "input": [
    {"url":"https://www.tiktok.com/@babyariel","num_of_posts":100,"post_type":"Video Posts","country":"US"}
  ]
}
```
...
- `url`: required HTTPS TikTok profile URL of the form `/@handle`, maximum 2048 characters.\n- `num_of_posts`: optional integer greater than or equal to zero.\n- `posts_to_not_include`: optional unique array of at most 500 non-empty strings, each at most 128 characters.\n- `what_to_collect`: optional string, maximum 128 characters.\n- `start_date` and `end_date`: optional valid `MM-DD-YYYY` strings or empty strings; when both exist, the end must be later.\n- `post_type`: optional `\"\"`, `\"Video Posts\"`, or `\"Image Posts\"`.\n- `sort_by`: optional string, maximum 64 characters.\n- `country`: optional ISO 3166-1 alpha-2 code or empty string.
