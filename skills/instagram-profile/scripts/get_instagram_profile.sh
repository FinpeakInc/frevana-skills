...
API_PATH="/service/instagram-profile"
CONNECT_TIMEOUT="10"
MAX_TIME="600"
...
usage() {
  cat <<'EOF'
Usage:
  get_instagram_profile.sh --profile-id VALUE [options] [--output /path/to/result.json] [--token "bearer token"]

Request options:
  --profile-id                 string parameter
...
python3 - "$PAYLOAD_FILE" '{"profile_id":"profile_id","profile-id":"profile_id","next_page_token":"next_page_token","next-page-token":"next_page_token","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"profile_id":"string","next_page_token":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["profile_id"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Instagram Profile API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Instagram Profile API returned an empty response body."
