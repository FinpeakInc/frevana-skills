...
API_PATH="/service/google-ai-overview"
CONNECT_TIMEOUT="10"
MAX_TIME="600"
...
usage() {
  cat <<'EOF'
Usage:
  get_google_ai_overview.sh --page-token VALUE [options] [--output /path/to/result.json] [--token "bearer token"]

Request options:
  --page-token                 string parameter
...
python3 - "$PAYLOAD_FILE" '{"page_token":"page_token","page-token":"page_token","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"page_token":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["page_token"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google AI Overview API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Google AI Overview API returned an empty response body."
