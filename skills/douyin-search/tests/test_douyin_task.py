...
class DouyinSkillTest(unittest.TestCase):
    ...
    def test_run_uses_correct_endpoint_token_and_preserves_result(self):
        ...
        if HOT_SEARCH:
            ...
            self.assertEqual(payload["max_results_per_query"], 100)
    ...
    def test_invalid_request_is_rejected_before_network(self):
        cases = (
            ("--keyword", "test", "--max-results-per-query", "0"),
            ("--keyword", "test", "--max-results-per-query", "2000"),
            tuple(arg for index in range(51) for arg in ("--keyword", f"keyword-{index}")),
            ("--keyword", "one", "--keyword", "two", "--max-results-per-query", "1000"),
        )
    ...
    def test_new_request_boundaries_are_accepted(self):
        cases = (
            ("--keyword", "test", "--max-results-per-query", "1999"),
            tuple(arg for index in range(50) for arg in ("--keyword", f"keyword-{index}"))
            + ("--max-results-per-query", "39"),
            ("--keyword", "same", "--keyword", "same", "--max-results-per-query", "1999"),
        )
        for options in cases:
            with self.subTest(options=options):
                result = self.run_script("create", *options)
                self.assertEqual(result.returncode, 0, result.stderr.decode())
        self.assertEqual(len(Handler.requests), len(cases))

    def test_default_explicitly_sends_ten_for_fifty_keywords(self):
        options = tuple(arg for index in range(50) for arg in ("--keyword", f"keyword-{index}"))
        result = self.run_script("create", *options)
        self.assertEqual(result.returncode, 0, result.stderr.decode())
        self.assertEqual(len(Handler.requests), 1)
        payload = json.loads(Handler.requests[0][3])
        self.assertEqual(len(payload["keywords"]), 50)
        self.assertEqual(payload["max_results_per_query"], 10)
    ...
    def test_git_bash_path_conversion_flow(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                ["bash", str(SCRIPT), "run", "--keyword", "test",
