...
description: Use when the user wants anchor text distribution from external backlinks/inbound links pointing to one target, such as top anchor phrases and counts.
---

# Backlinks Anchors

Call Frevana's `/service/backlinks/anchors` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **anchor text distribution for backlinks to one target, such as top anchor phrases and their backlink/referring-domain counts**.
...
```bash
bash <skill-path>/scripts/get_backlinks_anchors.sh --target frevana.com --limit 100
```
...
```bash
bash <skill-path>/scripts/get_backlinks_anchors.sh --target frevana.com --limit 100 \
  --output ./out/backlinks-anchors-result.json
```
...
```text
POST /service/backlinks/anchors
```
...
- Default file path: `./out/backlinks-anchors-<UTC timestamp>-<pid>.json`
