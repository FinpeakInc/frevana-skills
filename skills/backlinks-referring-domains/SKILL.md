...
description: Use when the user needs external referring domains that link to one target, with per-domain backlink metrics, filters, sorting, and pagination.
---

# Backlinks Referring Domains

Call Frevana's `/service/backlinks/referring-domains` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **the domains that link to one target, with per-domain backlink metrics, filters, sorting, and pagination**.
...
```bash
bash <skill-path>/scripts/get_backlinks_referring_domains.sh --target frevana.com --limit 100
```
...
```bash
bash <skill-path>/scripts/get_backlinks_referring_domains.sh --target frevana.com --limit 100 \
  --output ./out/backlinks-referring-domains-result.json
```
...
```text
POST /service/backlinks/referring-domains
```
...
- Default file path: `./out/backlinks-referring-domains-<UTC timestamp>-<pid>.json`
