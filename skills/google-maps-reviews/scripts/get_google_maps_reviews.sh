...
API_PATH="/service/google-maps-reviews"
CONNECT_TIMEOUT="10"
MAX_TIME="600"
...
usage() {
  cat <<'EOF'
Usage:
  get_google_maps_reviews.sh  [options] [--output /path/to/result.json] [--token "bearer token"]

Request options:
  --data-id                    string parameter
  --place-id                   string parameter
  --hl                         string parameter
  --sort-by                    string parameter
  --topic-id                   string parameter
...
python3 - "$PAYLOAD_FILE" '{"data_id":"data_id","data-id":"data_id","place_id":"place_id","place-id":"place_id","hl":"hl","sort_by":"sort_by","sort-by":"sort_by","topic_id":"topic_id","topic-id":"topic_id","query":"query","num":"num","next_page_token":"next_page_token","next-page-token":"next_page_token","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"data_id":"string","place_id":"string","hl":"string","sort_by":"string","topic_id":"string","query":"string","num":"int","next_page_token":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '[]' '["data_id","place_id"]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google Maps Reviews API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Google Maps Reviews API returned an empty response body."
