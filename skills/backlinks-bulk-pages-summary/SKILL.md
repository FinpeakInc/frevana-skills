---
name: backlinks-bulk-pages-summary
description: Use when the user wants aggregate page-level external backlink/inbound-link metrics for many domains or URLs in one batch request.
---

# Backlinks Bulk Pages Summary

Call Frevana's `/service/backlinks/bulk-pages-summary` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **aggregate page-level backlink metrics for many domains or URLs in one batch request**.
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_pages_summary.sh --targets frevana.com,example.com
```
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_pages_summary.sh --targets frevana.com,example.com \
  --output ./out/backlinks-bulk-pages-summary-result.json
```
...
```text
POST /service/backlinks/bulk-pages-summary
```

Supported request fields for this API:

`targets`, `include_subdomains`, `rank_scale`, `tag`
...
- Default file path: `./out/backlinks-bulk-pages-summary-<UTC timestamp>-<pid>.json`
