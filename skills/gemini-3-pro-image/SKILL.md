...
description: Use when the user wants Frevana-hosted image generation or editing with Gemini 3 Pro Image (gemini-3-pro-image), prioritizing high fidelity, complex scenes, photorealistic detail, and precise multi-reference editing.
---

# Gemini 3 Pro Image

Generate or edit images through Frevana's Gemini image endpoint using `gemini-3-pro-image`.
...
- Best suited for production-ready creative, high-fidelity photorealism, fine text and texture rendering, complex compositions, and demanding reference-based editing.
- For rapid prototyping, fast iterations, or high-volume generation where lower latency is preferred, route to `gemini-3.1-flash-image` instead.
...
bash <skill-path>/scripts/generate_image.sh \
  --prompt "A masterpiece architectural photo of a modern glass pavilion in a misty forest at dawn" \
  --aspect-ratio 16:9 \
  --image-size 4K \
  --candidate-count 1 \
  --temperature 0.7 \
  --output ./out/gemini-pro-result.json
```
...
```bash
bash <skill-path>/scripts/generate_image.sh \
  --prompt "Keep the product geometry and brand logo identical, but set it in a high-end luxury marble showroom" \
  --image ./refs/watch.png \
  --aspect-ratio 4:3 \
  --image-size 2K \
  --candidate-count 1 \
  --output ./out/gemini-pro-edit-result.json
```
