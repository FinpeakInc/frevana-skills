...
class GeminiProGenerateImageTests(unittest.TestCase):
    ...
    def test_rejects_invalid_model(self):
        result = self.run_script("--prompt", "test", "--model", "gemini-3.1-flash-image")
        self.assertNotEqual(result.returncode, 0)
...
with open(response_file, "w", encoding="utf-8") as f:
    json.dump({
        "generated_images": [{"image_url": "https://static.frevana.com/gemini-pro.png"}],
        "credits_consumed": 1,
        "captured_url": url,
...
""",
            ...
            env.update(
                {
                    ...
                    "FREVANA_AGENT_APP_INSTANCE_ID": "instance-pro-xyz",
                }
            )

            result = self.run_script(
                "--prompt", "A high-end luxury watch on granite",
                "--job-id", "job-pro-999",
                "--aspect-ratio", "4:3",
                "--image-size", "4K",
                "--candidate-count", "1",
                "--system-instruction", "Photorealistic details with micro reflections",
                "--temperature", "0.7",
                "--top-p", "0.9",
                "--top-k", "50",
                "--seed", "99999",
            ...
            self.assertIn("job_id=job-pro-999", payload["captured_url"])

            fields = payload["captured_fields"]
            self.assertEqual(fields["prompt"], "A high-end luxury watch on granite")
            self.assertEqual(fields["model"], "gemini-3-pro-image")
            self.assertEqual(fields["aspect_ratio"], "4:3")
            self.assertEqual(fields["image_size"], "4K")
            self.assertEqual(fields["candidate_count"], "1")
            self.assertEqual(fields["system_instruction"], "Photorealistic details with micro reflections")
            self.assertEqual(fields["temperature"], "0.7")
            self.assertEqual(fields["top_p"], "0.9")
            self.assertEqual(fields["top_k"], "50")
            self.assertEqual(fields["seed"], "99999")
            ...
            self.assertIn("x-frevana-agent-app-instance-id: instance-pro-xyz", headers)
...
with open(sys.argv[1], "w", encoding="utf-8") as f:
    json.dump({
        "generated_images": [{"image_url": "https://static.frevana.com/gemini-pro-edit.png"}],
        "credits_consumed": 1,
...
""",
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            payload = json.loads(result.stdout)
            self.assertEqual(payload["captured_model"], "gemini-3-pro-image")
