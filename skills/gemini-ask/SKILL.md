---
name: gemini-ask
description: Use when the user wants to ask Gemini a question through Frevana using the user's logged-in Chrome Extension session.
---

# Gemini Ask

Ask Gemini through the local Frevana daemon and Chrome Extension session.

## Purpose

This skill is for **asking Gemini a prompt** using the Chrome Extension-backed Frevana MCP tool `frevana_ask` with fixed provider `gemini`.

Inputs:

- `prompt` (required) - one or more questions or instructions to send to Gemini
...
- Gemini's answer returned by Frevana
...
- the user logged in to Gemini in Chrome
- `curl`
- `bash`
- `python3`
...
6. Run the Gemini ask only after setup succeeds.
7. If the call returns login/auth content or fails because Gemini is unavailable, tell the user they must be logged in to Gemini in Chrome.
...
```bash
bash <skill-path>/scripts/ask_gemini.sh \
...
bash <skill-path>/scripts/ask_gemini.sh \
  ...
  --format json \
  --output ./out/gemini-batch-answers.txt
```
...
Always send `provider: "gemini"`. Do not pass unsupported fields.
