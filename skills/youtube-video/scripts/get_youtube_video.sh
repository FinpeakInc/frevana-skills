...
API_PATH="/service/youtube-video"
CONNECT_TIMEOUT="10"
MAX_TIME="600"
...
usage() {
  cat <<'EOF'
Usage:
  get_youtube_video.sh --v VALUE [options] [--output /path/to/result.json] [--token "bearer token"]
...
python3 - "$PAYLOAD_FILE" '{"v":"v","gl":"gl","hl":"hl","next_page_token":"next_page_token","next-page-token":"next_page_token","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"v":"string","gl":"string","hl":"string","next_page_token":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["v"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "YouTube Video API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "YouTube Video API returned an empty response body."
