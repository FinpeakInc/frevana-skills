...
usage() {
  cat <<'EOF'
Usage:
  ask_doubao.sh --prompt "question 1" [--prompt "question 2"] [--prompt-file /path/to/prompts.txt] [--format text|json] [--timeout MS] [--output /path/to/result.txt]
...
if [[ -n "$OUTPUT_PATH" ]]; then
  ...
  echo "Saved Doubao answer to $OUTPUT_PATH" >&2
fi
