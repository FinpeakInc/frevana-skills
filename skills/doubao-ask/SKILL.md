---
name: doubao-ask
description: Use when the user wants to ask Doubao a question through Frevana using the user's logged-in Chrome Extension session.
---

# Doubao Ask

Ask Doubao through the local Frevana daemon and Chrome Extension session.

## Purpose

This skill is for **asking Doubao a prompt** using the Chrome Extension-backed Frevana MCP tool `frevana_ask` with fixed provider `doubao`.

Inputs:

- `prompt` (required) - one or more questions or instructions to send to Doubao
...
- Doubao's answer returned by Frevana
...
- the user logged in to Doubao in Chrome
- `curl`
- `bash`
- `python3`
...
6. Run the Doubao ask only after setup succeeds.
7. If the call returns login/auth content or fails because Doubao is unavailable, tell the user they must be logged in to Doubao in Chrome.
...
```bash
bash <skill-path>/scripts/ask_doubao.sh \
...
bash <skill-path>/scripts/ask_doubao.sh \
  ...
  --format json \
  --output ./out/doubao-batch-answers.txt
```
...
Always send `provider: "doubao"`. Do not pass unsupported fields.
...
- "Ask Doubao to explain quantum computing"
- "用 Doubao 回答这个问题：..."
