...
API_PATH="/service/google-images"
CONNECT_TIMEOUT="10"
MAX_TIME="600"
...
usage() {
  cat <<'EOF'
Usage:
  search_google_images.sh --q VALUE [options] [--output /path/to/result.json] [--token "bearer token"]
...
Request options:
  ...
  --chips                      string parameter
  --imgar                      string parameter
  --imgsz                      string parameter
  --safe                       string parameter
  --nfpr                       int parameter
  --filter                     int parameter
  --image-size                 string parameter
  --image-type                 string parameter
  --image-color                string parameter
  --licenses                   string parameter
  --period-unit                string parameter
  --period-value               int parameter
  --start-date                 string parameter
  --end-date                   string parameter
...
python3 - "$PAYLOAD_FILE" '{"q":"q","location":"location","uule":"uule","google_domain":"google_domain","google-domain":"google_domain","gl":"gl","hl":"hl","cr":"cr","device":"device","start":"start","ijn":"ijn","tbs":"tbs","chips":"chips","imgar":"imgar","imgsz":"imgsz","safe":"safe","nfpr":"nfpr","filter":"filter","image_size":"image_size","image-size":"image_size","image_type":"image_type","image-type":"image_type","image_color":"image_color","image-color":"image_color","licenses":"licenses","period_unit":"period_unit","period-unit":"period_unit","period_value":"period_value","period-value":"period_value","start_date":"start_date","start-date":"start_date","end_date":"end_date","end-date":"end_date","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"q":"string","location":"string","uule":"string","google_domain":"string","gl":"string","hl":"string","cr":"string","device":"string","start":"int","ijn":"int","tbs":"string","chips":"string","imgar":"string","imgsz":"string","safe":"string","nfpr":"int","filter":"int","image_size":"string","image_type":"string","image_color":"string","licenses":"string","period_unit":"string","period_value":"int","start_date":"string","end_date":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["q"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google Images Search API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Google Images Search API returned an empty response body."
