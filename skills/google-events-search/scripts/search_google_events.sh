...
API_PATH="/service/google-events"
CONNECT_TIMEOUT="10"
MAX_TIME="600"
...
usage() {
  cat <<'EOF'
Usage:
  search_google_events.sh --q VALUE [options] [--output /path/to/result.json] [--token "bearer token"]
...
python3 - "$PAYLOAD_FILE" '{"q":"q","location":"location","uule":"uule","gl":"gl","hl":"hl","start":"start","htichips":"htichips","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"q":"string","location":"string","uule":"string","gl":"string","hl":"string","start":"int","htichips":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["q"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google Events Search API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Google Events Search API returned an empty response body."
