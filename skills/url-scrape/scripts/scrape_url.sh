...
usage() {
  cat <<'EOF'
Usage:
  scrape_url.sh --url "https://example.com" [--provider url] [--timeout MS] [--output /path/to/result.md]

Options:
  --url        Absolute URL to scrape
  --provider   Optional scrape provider, default url
  --timeout    Optional Frevana tool timeout in milliseconds
  --output     Optional file path for saving the Frevana scrape result
...
if [[ -z "$PROVIDER" ]]; then
  echo "Invalid --provider value: provider cannot be empty." >&2
  exit 1
fi
...
export URL PROVIDER TOOL_TIMEOUT
...
if [[ -n "$OUTPUT_PATH" ]]; then
  ...
  echo "Saved URL scrape output to $OUTPUT_PATH" >&2
fi
