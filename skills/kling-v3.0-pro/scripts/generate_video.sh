...
DEFAULT_MODEL="kwaivgi/kling-v3.0-pro"
ALLOWED_MODELS=("kwaivgi/kling-v3.0-pro")
...
Options:
  ...
  --cfg-scale NUM        CFG scale guidance (e.g. 0.5)
...
if [[ -n "$DURATION" ]] && ! is_allowed "$DURATION" "${ALLOWED_DURATIONS[@]}"; then
  echo "Error: Invalid duration '$DURATION' for model $MODEL. Allowed durations: 3-15" >&2
  exit 1
fi
...
if neg:
    kling_params["negative_prompt"] = neg

cfg = os.environ.get("ENV_CFG_SCALE")
if cfg:
    kling_params["cfg_scale"] = float(cfg)

if kling_params:
    payload.setdefault("provider", {}).setdefault("options", {}).setdefault("kling", {}).update(kling_params)
