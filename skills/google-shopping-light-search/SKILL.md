...
Search Google Shopping Light product listings through Frevana.

## Purpose

This skill is for **finding lightweight Google Shopping product results**.
...
- validated response JSON with Google Shopping Light product results
...
### Basic shopping light search

```bash
bash <skill-path>/scripts/search_google_shopping_light.sh \
  --q "wireless earbuds"
```
...
### Search with pagination and device
...
bash <skill-path>/scripts/search_google_shopping_light.sh \
  ...
  --gl US \
  --hl en \
  --output ./out/google-shopping-light-search-result.json
```
...
Only `q` is required. Do not invent `google_domain`, `gl`, `hl`, `start`, or `device` values when the user did not provide them.
...
- Default file path: `./out/google-shopping-light-search-<UTC timestamp>-<pid>.json`
...
- This Light endpoint does not accept `sort_by`; use `google-shopping-search` if the user explicitly needs Google Shopping price sorting.
...
- "搜索 Google Shopping Light 上的 wireless earbuds"
- "查一下 Google Shopping Light 里 q=running shoes，gl=US，hl=en"
- "Google Shopping Light 搜索 wireless earbuds，device=mobile，并保存原始 JSON"

### English

- "Search Google Shopping Light for wireless earbuds"
- "Look up Google Shopping Light results for running shoes with gl=US and hl=en"
- "Search Google Shopping Light for wireless earbuds on mobile and save the raw JSON"
