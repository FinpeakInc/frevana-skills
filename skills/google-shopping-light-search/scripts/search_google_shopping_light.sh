...
usage() {
  cat <<'EOF'
Usage:
  search_google_shopping_light.sh --q "wireless earbuds" [--google-domain google.com] [--gl US] [--hl en] [--start 0] [--device desktop|mobile|tablet] [--output /path/to/result.json] [--token "bearer token"]
...
Options:
  ...
  --output            Optional file path for saving returned JSON. Defaults to ./out/google-shopping-light-search-<timestamp>-<pid>.json
...
if [[ -z "$OUTPUT_PATH" ]]; then
  OUTPUT_PATH="out/google-shopping-light-search-$(date -u +%Y%m%dT%H%M%SZ)-$$.json"
fi
...
export Q GOOGLE_DOMAIN GL HL START DEVICE
...
HTTP_CODE="$(
  curl -sS \
    ...
    -o "$RESPONSE_FILE" \
    -w "%{http_code}" \
    -X POST "$API_BASE_URL$GOOGLE_SHOPPING_LIGHT_SEARCH_PATH" \
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google Shopping Light search API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi

if [[ ! -s "$RESPONSE_FILE" ]]; then
  echo "Google Shopping Light search API returned an empty response body." >&2
  exit 1
fi
...
try:
    ...
    print(f"Google Shopping Light search API returned non-JSON response: {exc}", file=sys.stderr)
...
echo "Saved Google Shopping Light search JSON to $OUTPUT_PATH" >&2
