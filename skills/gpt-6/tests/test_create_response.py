...
class Gpt6CreateResponseTests(unittest.TestCase):
...
with open(response_file, "w", encoding="utf-8") as f:
    json.dump({
        ...
        "object": "response",
        "model": req.get("model"),
        "output_text": "Frontier response from GPT-6",
...
""",
            ...
            env.update(
                {
                    ...
                    "FREVANA_TOKEN": "gpt6-token",
                    "X_FREVANA_AGENT_APP_INSTANCE_ID": "instance-gpt6-pro",
                }
            )

            result = self.run_script(
                "--input", "Solve this complex theorem",
                "--instructions", "Show full mathematical derivation.",
            ...
            self.assertEqual(req["model"], "gpt-6")
            self.assertEqual(req["input"], "Solve this complex theorem")
            self.assertEqual(req["instructions"], "Show full mathematical derivation.")
            self.assertEqual(req["reasoning"], {"effort": "high"})
            self.assertEqual(req["service_tier"], "priority")

            headers = payload["captured_headers"]
            self.assertIn("Authorization: Bearer gpt6-token", headers)
            self.assertIn("x-frevana-agent-app-instance-id: instance-gpt6-pro", headers)
