...
class Gpt56TerraCreateResponseTests(unittest.TestCase):
...
with open(response_file, "w", encoding="utf-8") as f:
    json.dump({
        ...
        "object": "response",
        "model": req.get("model"),
        "output_text": "Balanced answer from terra",
...
""",
            ...
            env.update(
                {
                    ...
                    "FREVANA_TOKEN": "terra-token",
                    "FREVANA_AGENT_APP_INSTANCE_ID": "instance-terra-1",
                }
            )

            result = self.run_script(
                "--input", "Write a product announcement",
                "--instructions", "Tone: enthusiastic.",
                "--temperature", "0.7",
            ...
            self.assertEqual(req["input"], "Write a product announcement")
            self.assertEqual(req["instructions"], "Tone: enthusiastic.")
            self.assertEqual(req["temperature"], 0.7)
            self.assertEqual(req["max_output_tokens"], 2048)

            headers = payload["captured_headers"]
            self.assertIn("Authorization: Bearer terra-token", headers)
            self.assertIn("x-frevana-agent-app-instance-id: instance-terra-1", headers)
