---
name: backlinks-list
description: Use when the user needs individual external backlink/inbound-link rows pointing to one domain, subdomain, or URL, including filters, sorting, pagination, or live/lost selection.
---

# Backlinks List

Call Frevana's `/service/backlinks/backlinks` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **individual backlink rows for one domain, subdomain, or URL, including filtering, sorting, pagination, or live/lost backlink selection**.
...
```bash
bash <skill-path>/scripts/get_backlinks_list.sh --target frevana.com --limit 100
```
...
```bash
bash <skill-path>/scripts/get_backlinks_list.sh --target frevana.com --limit 100 \
  --output ./out/backlinks-list-result.json
```
...
```text
POST /service/backlinks/backlinks
```

Supported request fields for this API:

`target`, `mode`, `custom_mode`, `filters`, `order_by`, `offset`, `search_after_token`, `limit`, `backlinks_status_type`, `include_subdomains`, `include_indirect_links`, `exclude_internal_backlinks`, `rank_scale`, `tag`
...
- Default file path: `./out/backlinks-list-<UTC timestamp>-<pid>.json`
