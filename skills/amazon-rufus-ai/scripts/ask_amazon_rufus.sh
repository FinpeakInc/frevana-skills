...
usage() {
  cat <<'EOF'
Usage:
  ask_amazon_rufus.sh --url "https://www.amazon.com/dp/B0XXXXXXXX" --question "question" [--format text|json] [--timeout MS] [--output /path/to/result.txt]

Options:
  --url       Full Amazon product page URL containing /dp/<ASIN> or /gp/product/<ASIN>
  --question  Question to ask Amazon Rufus. May also use --prompt
  --prompt    Alias for --question
  --format    Output format: text or json. Default text
  --timeout   Optional Frevana tool timeout in milliseconds. Default 120000
  --output    Optional file path for saving the answer
...
if [[ -z "$QUESTION" ]]; then
  echo "Missing required argument: --question" >&2
  exit 1
fi
...
if [[ -n "$OUTPUT_PATH" ]]; then
  ...
  echo "Saved Amazon Rufus answer to $OUTPUT_PATH" >&2
fi
