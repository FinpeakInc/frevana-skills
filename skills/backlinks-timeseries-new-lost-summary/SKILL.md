---
name: backlinks-timeseries-new-lost-summary
description: Use when the user wants new versus lost external backlinks/inbound links or referring domains over time for one target and date range.
---

# Backlinks Timeseries New Lost Summary

Call Frevana's `/service/backlinks/timeseries-new-lost-summary` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **new versus lost backlinks or referring domains over time for one target and date range**.
...
```bash
bash <skill-path>/scripts/get_backlinks_timeseries_new_lost_summary.sh --target frevana.com --date-from 2026-06-01 --date-to 2026-07-01
```
...
```bash
bash <skill-path>/scripts/get_backlinks_timeseries_new_lost_summary.sh --target frevana.com --date-from 2026-06-01 --date-to 2026-07-01 \
  --output ./out/backlinks-timeseries-new-lost-summary-result.json
```
...
```text
POST /service/backlinks/timeseries-new-lost-summary
```

Supported request fields for this API:

`target`, `date_from`, `date_to`, `group_range`, `include_subdomains`, `tag`
...
- Default file path: `./out/backlinks-timeseries-new-lost-summary-<UTC timestamp>-<pid>.json`
