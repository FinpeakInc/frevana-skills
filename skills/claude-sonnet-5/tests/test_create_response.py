...
class ClaudeSonnet5CreateResponseTests(unittest.TestCase):
...
""",
            ...
            res1 = self.run_script("--input", "hi", "--model", "claude-sonnet-5", env=env)
            self.assertEqual(res1.returncode, 0, res1.stderr)
            self.assertEqual(json.loads(res1.stdout)["model"], "anthropic/claude-sonnet-5")
...
""",
            ...
            self.assertEqual(req["model"], "anthropic/claude-sonnet-5")
