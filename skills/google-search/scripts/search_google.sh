...
usage() {
  cat <<'EOF'
Usage:
  search_google.sh --q "coffee" [--location "Austin, Texas, United States"] [--gl us] [--hl en] [--num 10] [--start 0] [--safe active|off] [--device desktop|mobile|tablet] [--output /path/to/result.json] [--token "bearer token"]

Options:
  --q           Search query keyword to send
  --query       Alias for --q
  --location    Optional search origin location
  --gl          Optional country/region code
  --hl          Optional language code
  --num         Optional result count, 1 through 100
  --start       Optional pagination start offset, 0 or greater
  --safe        Optional safe search level: active or off
  --device      Optional device type: desktop, mobile, or tablet
  --output      Optional file path for saving returned JSON. Defaults to ./out/google-search-<timestamp>-<pid>.json
...
if [[ -n "$SAFE" && "$SAFE" != "active" && "$SAFE" != "off" ]]; then
  echo "Invalid --safe value: $SAFE. Allowed values: active, off" >&2
  exit 1
fi
...
if [[ -n "$NUM" ]]; then
  if ! [[ "$NUM" =~ ^[0-9]+$ ]]; then
    echo "Invalid --num value: $NUM. Expected an integer from 1 through 100." >&2
    exit 1
  fi
...
if [[ -z "$OUTPUT_PATH" ]]; then
  OUTPUT_PATH="out/google-search-$(date -u +%Y%m%dT%H%M%SZ)-$$.json"
fi
...
export Q LOCATION GL HL NUM START SAFE DEVICE
...
HTTP_CODE="$(
  curl -sS \
    ...
    -o "$RESPONSE_FILE" \
    -w "%{http_code}" \
    -X POST "$API_BASE_URL$GOOGLE_SEARCH_PATH" \
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google Search API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi

if [[ ! -s "$RESPONSE_FILE" ]]; then
  echo "Google Search API returned an empty response body." >&2
  exit 1
fi
...
try:
    ...
    print(f"Google Search API returned non-JSON response: {exc}", file=sys.stderr)
...
echo "Saved Google Search JSON to $OUTPUT_PATH" >&2
