---
name: backlinks-bulk-spam-score
description: Use when the user wants external backlink spam score or inbound-link risk metrics for many domains or URLs in one batch request.
---

# Backlinks Bulk Spam Score

Call Frevana's `/service/backlinks/bulk-spam-score` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **spam score or backlink risk metrics for many domains or URLs in one batch request**.
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_spam_score.sh --targets frevana.com,example.com
```
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_spam_score.sh --targets frevana.com,example.com \
  --output ./out/backlinks-bulk-spam-score-result.json
```
...
```text
POST /service/backlinks/bulk-spam-score
```
...
- Default file path: `./out/backlinks-bulk-spam-score-<UTC timestamp>-<pid>.json`
