---
name: backlinks-domain-pages-summary
description: Use when the user wants aggregate page-level external backlink/inbound-link metrics for one target, without fetching the full list of pages.
---

# Backlinks Domain Pages Summary

Call Frevana's `/service/backlinks/domain-pages-summary` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **aggregate page-level backlink metrics for one target, without fetching the full list of pages**.
...
```bash
bash <skill-path>/scripts/get_backlinks_domain_pages_summary.sh --target frevana.com --limit 100
```
...
```bash
bash <skill-path>/scripts/get_backlinks_domain_pages_summary.sh --target frevana.com --limit 100 \
  --output ./out/backlinks-domain-pages-summary-result.json
```
...
```text
POST /service/backlinks/domain-pages-summary
```

Supported request fields for this API:

`target`, `limit`, `offset`, `internal_list_limit`, `backlinks_status_type`, `filters`, `order_by`, `backlinks_filters`, `include_subdomains`, `include_indirect_links`, `exclude_internal_backlinks`, `rank_scale`, `tag`
...
- Default file path: `./out/backlinks-domain-pages-summary-<UTC timestamp>-<pid>.json`
