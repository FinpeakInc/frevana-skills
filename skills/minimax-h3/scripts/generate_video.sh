...
ALLOWED_MODELS=("minimax/hailuo-3")
...
if [[ "$GENERATE_AUDIO" == "true" ]] && [[ "$SUPPORTS_AUDIO" != "true" ]]; then
  echo "Error: Model $MODEL does not support audio generation." >&2
  exit 1
fi

# Load prompt if prompt-file
...
# Helper function to encode image URL or local file to data URI
format_image_source() {
  local src="$1"
  python3 - "$src" <<'PY'
