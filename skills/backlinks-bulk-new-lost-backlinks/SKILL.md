---
name: backlinks-bulk-new-lost-backlinks
description: Use when the user wants new and lost external backlink/inbound-link count totals for many domains or URLs in one batch request.
---

# Backlinks Bulk New Lost Backlinks

Call Frevana's `/service/backlinks/bulk-new-lost-backlinks` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **new and lost backlink count totals for many domains or URLs in one batch request**.
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_new_lost_backlinks.sh --targets frevana.com,example.com --date-from 2026-06-01
```
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_new_lost_backlinks.sh --targets frevana.com,example.com --date-from 2026-06-01 \
  --output ./out/backlinks-bulk-new-lost-backlinks-result.json
```
...
```text
POST /service/backlinks/bulk-new-lost-backlinks
```
...
- Default file path: `./out/backlinks-bulk-new-lost-backlinks-<UTC timestamp>-<pid>.json`
