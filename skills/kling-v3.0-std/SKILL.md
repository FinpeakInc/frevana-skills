---
name: kling-v3.0-std
description: Generate smooth, high-quality video with synchronized audio using Kling Video v3.0 Standard (kwaivgi/kling-v3.0-std) via Frevana's OpenRouter video endpoint, supporting 720p resolution, durations from 3 to 15 seconds, negative prompt, cfg scale, and first/last frame controls.
---

# Kling Video v3.0 Standard

Generate smooth video with synchronized audio through Frevana's OpenRouter video endpoint using `kwaivgi/kling-v3.0-std`.
...
bash skills/kling-v3.0-std/scripts/generate_video.sh \
  --prompt "A cyberpunk sports car drifting through neon rain-slicked streets at night" \
  --duration 5 \
  --aspect-ratio 16:9 \
  --download-dir ./output
...
bash skills/kling-v3.0-std/scripts/generate_video.sh \
  --prompt "The camera pushes forward as smoke curls into the air" \
  --first-frame ./scene.jpg \
  --duration 5 \
  --download-dir ./output

# Status check
bash skills/kling-v3.0-std/scripts/generate_video.sh status --job-id <JOB_ID>
```
...
- `--model MODEL`: Model slug (default: `kwaivgi/kling-v3.0-std`)
