...
usage() {
  cat <<'EOF'
Usage:
  search_google_ads.sh --q VALUE --location VALUE [options] [--output /path/to/result.json] [--token "bearer token"]
...
python3 - "$PAYLOAD_FILE" '{"q":"q","location":"location","hl":"hl","safe":"safe","nfpr":"nfpr","device":"device","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"q":"string","location":"string","hl":"string","safe":"string","nfpr":"int","device":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["q","location"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "Google Ads Search API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "Google Ads Search API returned an empty response body."
