...
usage() {
  cat <<'EOF'
Usage:
  ask_perplexity.sh --prompt "question 1" [--prompt "question 2"] [--prompt-file /path/to/prompts.txt] [--format text|json] [--timeout MS] [--output /path/to/result.txt]

Options:
  --prompt       Prompt/question to send. May be repeated.
...
if [[ -n "$OUTPUT_PATH" ]]; then
  ...
  echo "Saved Perplexity answer to $OUTPUT_PATH" >&2
fi
