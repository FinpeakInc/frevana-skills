...
RESULT = b'[{"ok":true,"source":"tiktok-posts-collect-by-url"}]'
...
class TikTokSkillTest(unittest.TestCase):
    ...
    def test_run_creates_polls_and_preserves_raw_result(self):
        ...
        self.assertEqual(
            ...
            [
                ("POST", "/service/tiktok/posts/collect-by-url"),
        ...
        self.assertEqual(payload["input"], [{"url":"https://www.tiktok.com/@mrbeast/video/7553327774983802143","country":"US"}])
    ...
    def test_invalid_operation_input_is_rejected_before_network(self):
        result = self.run_script("create", "--input-json", '{"url":"https://www.tiktok.com/@mrbeast"}')
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("valid HTTPS TikTok post URL", result.stderr.decode())
