---
name: tiktok-posts-collect-by-url
description: Collect public TikTok post or video records, details, and metadata from one or more exact TikTok post URLs through Frevana, and continue status or result retrieval for tasks created by this operation. Use when exact video or post links are provided; do not use for discovery, profile-detail collection, or TikTok Ads management.
---

# TikTok Posts Collect by URL

Use this skill to collect public post records from one or more exact TikTok post URLs. The bundled script owns the complete create, poll, and raw-result flow.

## Required input

Every input object requires TikTok post `url`. Supply one to twenty objects through repeatable `--input-json` or one `--input-file` containing a JSON array.
...
bash {baseDir}/scripts/tiktok_task.sh run \
  --input-json '{"url":"https://www.tiktok.com/@mrbeast/video/7553327774983802143","country":"US"}'
...
- This skill fixes creation to `POST /service/tiktok/posts/collect-by-url`; do not route another TikTok operation through it.
