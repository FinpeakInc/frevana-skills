---
name: backlinks-bulk-ranks
description: Use when the user wants external backlink authority/rank metrics for many domains or URLs in one request, without detailed link rows.
---

# Backlinks Bulk Ranks

Call Frevana's `/service/backlinks/bulk-ranks` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **authority/rank metrics for many domains or URLs in one request, without detailed backlink rows**.
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_ranks.sh --targets frevana.com,example.com
```
...
```bash
bash <skill-path>/scripts/get_backlinks_bulk_ranks.sh --targets frevana.com,example.com \
  --output ./out/backlinks-bulk-ranks-result.json
```
...
```text
POST /service/backlinks/bulk-ranks
```

Supported request fields for this API:

`targets`, `rank_scale`, `tag`
...
- Default file path: `./out/backlinks-bulk-ranks-<UTC timestamp>-<pid>.json`
