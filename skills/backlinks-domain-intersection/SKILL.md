...
description: Use when the user wants to compare multiple domains and find external referring domains that link to all or selected targets, with optional exclusions.
---

# Backlinks Domain Intersection

Call Frevana's `/service/backlinks/domain-intersection` endpoint and return the Backlinks API response unchanged.

## Purpose

This skill is for **comparing multiple domains and finding referring domains that link to all or selected targets, with optional exclusions**.
...
```bash
bash <skill-path>/scripts/get_backlinks_domain_intersection.sh --targets-json '{"site1":"example.com","site2":"frevana.com"}' --limit 100
```
...
```bash
bash <skill-path>/scripts/get_backlinks_domain_intersection.sh --targets-json '{"site1":"example.com","site2":"frevana.com"}' --limit 100 \
  --output ./out/backlinks-domain-intersection-result.json
```
...
```text
POST /service/backlinks/domain-intersection
```

Supported request fields for this API:

`targets`, `exclude_targets`, `filters`, `order_by`, `offset`, `limit`, `internal_list_limit`, `backlinks_status_type`, `backlinks_filters`, `include_subdomains`, `include_indirect_links`, `exclude_internal_backlinks`, `intersection_mode`, `rank_scale`, `tag`
...
- Default file path: `./out/backlinks-domain-intersection-<UTC timestamp>-<pid>.json`
