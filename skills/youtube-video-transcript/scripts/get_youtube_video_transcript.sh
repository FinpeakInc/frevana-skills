...
usage() {
  cat <<'EOF'
Usage:
  get_youtube_video_transcript.sh --v VALUE [options] [--output /path/to/result.json] [--token "bearer token"]

Request options:
  --v                          string parameter
  --language-code              string parameter
...
python3 - "$PAYLOAD_FILE" '{"v":"v","language_code":"language_code","language-code":"language_code","title":"title","type":"type","no_cache":"no_cache","no-cache":"no_cache","async":"async","zero_trace":"zero_trace","zero-trace":"zero_trace"}' '{"v":"string","language_code":"string","title":"string","type":"string","no_cache":"bool","async":"bool","zero_trace":"bool"}' '["v"]' '[]' "${PAYLOAD_ARGS[@]}" <<'PY'
import json
import sys
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  echo "YouTube Video Transcript API request failed with HTTP $HTTP_CODE" >&2
  cat "$RESPONSE_FILE" >&2
  exit 1
fi
[[ -s "$RESPONSE_FILE" ]] || fail "YouTube Video Transcript API returned an empty response body."
