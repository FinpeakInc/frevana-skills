...
ALLOWED_MODELS=("google/veo-3.1" "google/veo-3.1-fast" "google/veo-3.1-lite")
...
Options:
  ...
  --model MODEL          Model slug (default: $DEFAULT_MODEL, allowed: ${ALLOWED_MODELS[*]})
  ...
  --person-generation VAL Person generation policy (e.g. allow_adult, dont_allow)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --person-generation) PERSON_GENERATION="${2:-}"; shift 2 ;;
    ...
    --api-base-url) API_BASE_URL="${2:-}"; while [[ "$API_BASE_URL" == */ ]]; do API_BASE_URL="${API_BASE_URL%/}"; done; [[ "$API_BASE_URL" =~ ^https?://[^/?#]+(/[^?#]*)?$ ]] || { echo "Invalid API base URL after normalisation: ${API_BASE_URL:-<empty>}" >&2; exit 1; }; shift 2 ;;
    -h|--help) usage; exit 0 ;;
    *)
...
if [[ "$MODEL" == "google/veo-3.1-lite" ]]; then
...
if [[ ${#REFERENCE_IMAGES[@]} -gt 0 ]] && ! is_allowed "image_url" ${SUPPORTED_INPUT_REFERENCES[@]+"${SUPPORTED_INPUT_REFERENCES[@]}"}; then
  echo "Error: Model $MODEL does not support image input references." >&2
  exit 1
fi
if [[ ${#REFERENCE_AUDIOS[@]} -gt 0 ]] && ! is_allowed "audio_url" ${SUPPORTED_INPUT_REFERENCES[@]+"${SUPPORTED_INPUT_REFERENCES[@]}"}; then
  echo "Error: Model $MODEL does not support audio input references." >&2
  exit 1
fi
if [[ ${#REFERENCE_VIDEOS[@]} -gt 0 ]] && ! is_allowed "video_url" ${SUPPORTED_INPUT_REFERENCES[@]+"${SUPPORTED_INPUT_REFERENCES[@]}"}; then
  echo "Error: Model $MODEL does not support video input references." >&2
  exit 1
fi
for reference_url in ${REFERENCE_AUDIOS[@]+"${REFERENCE_AUDIOS[@]}"} ${REFERENCE_VIDEOS[@]+"${REFERENCE_VIDEOS[@]}"}; do
  [[ -z "$reference_url" || "$reference_url" == http://* || "$reference_url" == https://* ]] || { echo "Error: Audio and video references must be HTTP(S) URLs." >&2; exit 1; }
done
...
if [[ "$COMMAND" == "create" ]]; then
  ...
  ENV_PERSON_GENERATION="$PERSON_GENERATION" \
...
provider_params = {}
neg = os.environ.get("ENV_NEGATIVE_PROMPT")
if neg:
    provider_params["negativePrompt"] = neg

pg = os.environ.get("ENV_PERSON_GENERATION")
if pg:
    provider_params["personGeneration"] = pg

if provider_params:
    payload.setdefault("provider", {}).setdefault("options", {}).setdefault("google-vertex", {}).setdefault("parameters", {}).update(provider_params)
