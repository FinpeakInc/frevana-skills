...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --api-base-url) API_BASE_URL="${2:-}"; while [[ "$API_BASE_URL" == */ ]]; do API_BASE_URL="${API_BASE_URL%/}"; done; [[ -n "$API_BASE_URL" ]] || { echo "API base URL is empty after normalisation" >&2; exit 1; }; shift 2 ;;
    -h|--help) usage; exit 0 ;;
    *)
