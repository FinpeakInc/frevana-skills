...
usage() {
  cat <<'EOF'
Usage:
  publish_linkedin_post.sh --text "post text" [--format text|json] [--timeout MS] [--output /path/to/result.txt]
  publish_linkedin_post.sh --mode article --title "Article title" --text-file ./article.txt [--cover-image URL] [--timeout MS]

Options:
  --text         Final post or article text. Mutually exclusive with --text-file
  --text-file    UTF-8 text file containing final post/article text
  --mode         LinkedIn mode: post or article. Default post
  --title        LinkedIn article title. Required when --mode article
  --cover-image  LinkedIn article cover image URL or data URL
...
case "$MODE" in
  post|article) ;;
  *)
    echo "Invalid --mode value: $MODE. Allowed values: post, article" >&2
    exit 1
    ;;
esac

if [[ "$MODE" == "article" && -z "$TITLE" ]]; then
  echo "Missing required argument: --title is required for linkedin article mode." >&2
  exit 1
fi
...
export PROVIDER TEXT MODE TITLE COVER_IMAGE TOOL_TIMEOUT
...
if os.environ.get("COVER_IMAGE"):
    payload["cover_image"] = os.environ["COVER_IMAGE"]
...
if [[ -n "$OUTPUT_PATH" ]]; then
  ...
  echo "Saved LinkedIn publish result to $OUTPUT_PATH" >&2
fi
